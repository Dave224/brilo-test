...
{
    global $wp_rewrite;
    $wp_rewrite->pagination_base = __("strana", "WPA_DOMAIN"); //where new-slug is the slug you want to use
...
function panda_rename_page_text($link_output, $link)
{
    if (Util::stringStartsWith($link["text"], "Page")) {
        $link_output = str_replace("Page", __("Strana", "WPA_DOMAIN"), $link_output);
    }
    return $link_output;
}
